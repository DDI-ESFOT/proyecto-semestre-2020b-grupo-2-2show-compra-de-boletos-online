...
import ComprarEntrada from "../pages/ByTickesPage";
...
export default function EventsPage() {
  ...
  React.useEffect(() => {
    ...
    const obtenerEventos = async () => {
      try {
        await db.collection("eventos").onSnapshot((querySnapshot) => {
          const eventList = [];
          ...
          {eventos.map((event) => {
            return (
              <Card
                hoverable
                style={{ width: 240 }}
                cover={<img alt="example" src={event.bannerEvento} />}
              >
                <Meta
          ...
          {listaEventos.map(({ event }) => {
            return (
              <Card
                ...
                cover={<img alt="example" src={event.bannerEvento} />}
              >
                <Meta title={event.nomEvento} description="proximamente" />
